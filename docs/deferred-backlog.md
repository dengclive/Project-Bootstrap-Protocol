...
| J-17 | **A `never_read_paths` entry can inject shell into the emitted hook.** CONFIRMED by execution (round-3 lens C): a pattern of `PAT_EOF` terminates the quoted heredoc that carries the pattern list, and subsequent entries become command lines in `secrets-gate.sh`. A pattern of `touch /tmp/x` then runs on every hook invocation. With a non-executable pattern the ERR trap converts it into a permanent `rc=2` on every payload — a total DoS no test catches. Present at **every commit in this chain**, so not introduced by any review round, but unrecorded until now and in the same emit path as the original P0-1 RCE. `lib/installer.py` validates `never_read_paths` nowhere. The config is operator-authored, so this is privilege-equivalent rather than a privilege escalation — but the installer should still refuse a pattern containing the heredoc sentinel, a newline, or a leading `-` | `open` — cheap to fix (validate at `resolve_config`), and it is the last unvalidated path into a generated hook body |
| J-18 | **`eval-gate`'s two substrates diff different ranges.** The shell diffs `HEAD~1`; the SDK diffs `@{u}..HEAD`. With two unpushed commits and the prompt file in the OLDER one, shell allows and SDK denies — a binding-rule violation, present at every commit in this chain and in both directions depending on history shape. The differential's eval-gate section has no `deny` rows at all, so it can see neither | `open` |
| J-19 | **The empty-payload refusal overrides a hook's declared posture.** The `hook_fail` on empty stdin sits in the shared header ABOVE each hook's `FAIL_CLOSED=0` line, so `cost-log` (Stop), `task-done-alarm` (SubagentStop), `decision-required-alarm` (Notification) and `drift-detector` (PostToolUse) all exit 2 on an empty payload despite declaring themselves advisory. `exit 2` does not mean "fail closed" on a `Stop` event — it means "do not stop" — so one exit code was applied across five event semantics. Consequence is reasoned, not executed: no live Claude Code runtime was available to any round-3 lens | `open` |
| J-20 | **No test compares the tokenizers against a real shell.** Every one of the five review rounds has shipped a `cmd_segments` / `_sg_scan` defect, and every round's clean tokenizer result came from a hand-built corpus written by the same person who wrote the code — round 3's lens A swept 2202 generated commands and found nothing, then found two fail-opens by hand, because the generated corpus contained no multi-line quoted runs. A generative differential against `bash` word-splitting over a few hundred thousand random commands is the only mechanism any lens could name that reaches the class | `open` — highest-value build item on this list |
| J-15 | **A nested quote inside a shell invoker's argument keeps its quote character on the word.** The round-2 quoted-run rule re-tokenizes an invoker's argument (`sh -c`, `bash -c`, `ssh`, …) so `sh -c 'cat secrets/prod.yaml'` blocks again, but the re-tokenization is a whitespace split, not a second parse — so `ssh h "git commit -m '.env'"` yields `'.env'` and over-matches. Over-match on a command that runs a shell *remotely* is the cheap direction, and the frequent case (ordinary prose in a `git commit -m`) is protected by the invoker test, which `git` fails. Closing it means recursing the tokenizer into the run, which re-opens the question of how deep to follow nesting | `no-action` — recorded trade-off; revisit only with a real operator report |
