...
- **`implementer`** — `model: sonnet`, `isolation: worktree` **when `commands.execute_in_cwd` is true (W-1)**. Retrofit weights this higher than greenfield: an established codebase is far likelier to reach its toolchain through a container than a fresh one. If the project's test/lint commands go through a fixed mount (`docker compose exec`, `kubectl exec`, `ssh`, a devcontainer CLI) they land in a tree the mount chose, not the worktree the implementer is standing in — so the gate would compile and test the main checkout and pass on code that was never built. Answer the retrofit interview's **Command execution location** question honestly; `false` drops the `isolation:` line and starts `max_concurrent_tasks` at 1. Override with `workflow.implementer_isolation` (`auto` | `worktree` | `none`) in `bootstrap.config.yaml` — never by hand-editing the emitted `implementer.md`, which trips the digest guard and makes the file `SKIP` forever. **Retrofit-specific instructions:**
