...
**The cap is deliberately absent from the primed slice [LIT-07].** Enforcement is the harness's job; a prompt-visible budget invites rationing against it (the budget-anxiety lineage the v2.3.0 note cites). Redaction reduces prompt *salience* only — the cap stays truthful and workspace-readable in the committed task file, in committed `loop-config.md`/`goal-config.md`, and in `learnings/mode-selection.md`'s iterations-vs-cap entries [LIT-08] — which is defensible non-disclosure consistent with the safety-net/success-signal split above, not concealment. The why-redacted rationale is recorded operator-visibly in the config comment blocks so a future maintainer can tell this deliberate absence from an oversight and does not "fix" the unfiltered task definition back into priming. Recorded in the [Assumption Ledger](#assumption-ledger): calibrated against the current tier; a model generation that plans *better* with a known budget flips this default.
