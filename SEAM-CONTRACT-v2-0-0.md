...
binds:
  ...
  tessera_prd: ">=0.5.0,<0.6.0"           # [D-03; carried at seam 2.0.0] Floor = first Tessera version that ABSORBS the seam-1.0.0 fold (targets_seam_version + §12.4 estimate semantics). RANGE UNCHANGED at seam 2.0.0: the Tessera version that absorbs the substrate re-cut (targets_seam_version: 2.0.0 + a runner consuming build_hooks per §3.3) is a Tessera-roadmap fact not yet pinnable here, so the floor is carried forward rather than invented. Until that Tessera fold lands, this binds set is KNOWINGLY UNSATISFIED and CI check 0 SHOULD fail on both halves (targets_seam_version 1.2.0≠2.0.0, and no absorbing version yet) — the designed out-of-sync signal, not a bug (P-5). The owner re-cuts this floor when the absorbing Tessera version is known.
...
**[seam 2.0.0 — the §9 deferred surface, now live.]** At the substrate release (protocol 2.1.0, IC-5), the enforcement layer under `gate_substrate: "sdk-callable"` is carried by a protocol-emitted, project-resident Python module at `.claude/sdk_gates/gates.py`. Its public builder is a **seam surface** — Tessera's own subprocess dispatch runner (§9 "Runner ownership — Tessera-owned") imports it *inside the dispatch subprocess only* (§2 non-import rule; never in Tessera's core process) to construct the Agent SDK hook configuration.

**Locked builder signature (resolved at this bump from the §9 proposal):**

- `build_hooks(config: dict) -> dict` — the module's **only** public symbol. Takes the resolved project config (the dict the protocol resolves from `bootstrap.config.yaml`) and returns the Agent SDK hooks mapping, shape `{"PreToolUse": [HookMatcher(...), ...], ...}` (event name → list of `claude_agent_sdk.HookMatcher`).
...
The **shell substrate remains selectable** (`gate_substrate: "shell"`, the emitted `hooks/*` scripts of §7.2). Both gate carriers are contract-level; which one a given install uses is the protocol's `gate_substrate` state field, not a Tessera choice.
...
- **Security-critical set** — `hooks/secrets-gate*`, `hooks/spec-gate-commit*`, `hooks/dependency-gate*`, `hooks/test-gate*`, `hooks/eval-gate*`, `hooks/tdd-gate*`, `hooks/format-lint-gate*`, `settings.json`, and **[seam 2.0.0] `sdk_gates/gates.py`**. *`spec-gate-entry` is deliberately excluded: it is warn-tier by the protocol's own entry-warn/commit-block split; the block-tier `spec-gate-commit` is the critical one.* **[seam 2.0.0] Substrate-dependent membership:** under `gate_substrate: "sdk-callable"` the single `.claude/sdk_gates/gates.py` module carries EVERY gate (§9), so it is security-critical — the strictest tier, never non-critical (a hand-edit there silently rewrites all enforcement). Under `gate_substrate: "shell"` the per-hook scripts above are the carriers and `gates.py` is not emitted. The union is listed here; check 8 (§8.2) asserts the set that matches the installed substrate.
...
8. The security-critical AND autonomy-critical hook sets (§7.2) are present and match the contract lists. **[seam 2.0.0] Substrate-dependent:** assert the set matching the installed `gate_substrate` — under `"sdk-callable"`, `.claude/sdk_gates/gates.py` is present, manifest-tracked, and in the security-critical tier (and the per-hook shell scripts are absent); under `"shell"`, the per-hook `hooks/*` scripts are present and `gates.py` is absent. Asserting the wrong-substrate set is itself a check failure.
