...
- `build_hooks(config: dict) -> dict` — the module's **sole public builder** (every other symbol is underscore-private). Takes the resolved project config (the dict the protocol resolves from `bootstrap.config.yaml`) and returns the Agent SDK hooks mapping, shape `{"PreToolUse": [HookMatcher(...), ...], ...}` (event name → list of `claude_agent_sdk.HookMatcher`).
...
- **Coverage.** `build_hooks` wires the **security-critical `PreToolUse` gates** (`secrets-gate`, `spec-gate-commit`, `dependency-gate`, `test-gate`, `eval-gate`, `tdd-gate`, `format-lint-gate` — the enabled subset for the resolved config). It does **not** carry the autonomy-critical hooks (`drift-detector-loop-cooperation`, `iteration-summary-enforcement`) or the observability/alarm hooks (`ci-mirror`, `cost-log`, `drift-detector`, the `*-alarm` hooks); those remain shell-emitted regardless of substrate (§7.2).
...
**Emission vs activation (the corrected model — `gate_substrate` selects the active carrier, not which files exist).** `.claude/sdk_gates/gates.py` is emitted by **every greenfield install** and is manifest-tracked security-critical — its presence is **not** conditional on `gate_substrate`. The `gate_substrate` state field (`"shell"` by default; `"sdk-callable"` grantable only behind the IC gate) selects which carrier is **active** (wired into `settings.json`), never which files land on disk: the shell `hooks/*` scripts and `gates.py` **coexist** in a greenfield tree, both manifest-tracked and security-critical. (The retrofit overlay is the one path that drops `gates.py`.) "Shell vs sdk-callable" is therefore an activation choice, never an emit-XOR.
