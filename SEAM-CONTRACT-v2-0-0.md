...
binds:
  ...
  claude_code_runtime: ">=2.1.210"        # [v1.0.0; confirmed v1.2.0 2026-07-17] min runtime: below it a PreToolUse hook timeout is misreported to the model as a user rejection and an unattended session stalls (§B-14) + worktree-entry consent ≥2.1.206 + hyphen matchers ≥2.1.195; 2.1.210 is the binding floor. [CORRECTED 2026-08-13, X-51: this comment used to say "min runtime for fail-closed PreToolUse timeout" and "CONFIRMED ... the fail-closed PreToolUse hook-timeout fix lands at exactly 2.1.210". NO RUNTIME DELIVERS FAIL-CLOSED TIMEOUTS — measured live at v2.1.227, a cancelled hook exits 124/137/143, only exit 2 blocks, and the call proceeds. The floor is KEPT for the no-stall behaviour, which is what 2.1.210 actually fixed.] The two subsumed floors verified in place. The IC-6 native-worktree flag introduction version is not officially pinnable but is subsumed by this floor.
...
- **Coverage.** `build_hooks` wires the seven **security-critical** gates enabled for the resolved config, across two events: **six `PreToolUse` denies** (`secrets-gate`, `spec-gate-commit`, `dependency-gate`, `test-gate`, `eval-gate`, `tdd-gate`) plus **`format-lint-gate` as a `PostToolUse` advisory** (matcher `Write|Edit`, non-blocking — it returns a `systemMessage`, never a deny; the "deny shape" guarantee below applies only to the six blocking gates). Matchers: `secrets-gate` is registered **twice** — `Read|Write|Edit|NotebookEdit|Grep|Glob` and `Bash` — mirroring the shell suite's `settings.json`; `spec-gate-commit`, `dependency-gate`, `test-gate` and `eval-gate` take `Bash`; `tdd-gate` takes `Write`. So a gate count and a `HookMatcher` count are not the same number. **[Descriptive correction, 2026-07-28, lens A F7 — recorded per the DR-03 heading/prose-staleness discipline, not a wire change, so it rides inside the current version.]** This line previously read `Read|Write|Edit`/`Bash`/`Write`, which was the pre-P0-2 wiring; worse, the emitted module's matcher table matched the stale prose rather than the shipped `settings.json`, so under `gate_substrate: "sdk-callable"` the shell-command route to a never-read path was carried by neither. The §8.4 trigger walk for this change is recorded in `docs/changelog.md`: no trigger fires — §7.2 membership is keyed on gate **name** and all seven names are unchanged, and §8.4's closing line ("changes that touch only gate internals or dispatch policy do not bump `seam_version`") governs the remainder. It does **not** carry the autonomy-critical hooks (`drift-detector-loop-cooperation`, `iteration-summary-enforcement`) or the observability/alarm hooks (`ci-mirror`, `cost-log`, `drift-detector`, the `*-alarm` hooks); those remain shell-emitted regardless of substrate (§7.2).
...
0. **`binds` resolution (the sync assertion) [rewritten at seam 1.0.0 for the re-cut key set (D-04)].** Each consumer's declared version resolves inside its `binds` range (§8.1a), AND each consumer's `targets_seam_version` equals this artifact's `seam_version`. Both halves must hold; a failure here is the designed out-of-sync signal (P-5) — a deliberately-failing check 0 during a consumer-fold window is the *honest* state, and a passing check 0 immediately after a MAJOR is itself suspicious. Concretely, for every key in `binds`:
   ...
   - `claude_code_runtime`: assert the installed Claude Code CLI version satisfies the behavioral floor (no unattended stall on hook timeout, worktree consent, matcher semantics). **[Corrected 2026-08-13, X-51 — this read "fail-closed hook timeouts", which no runtime provides.]**
