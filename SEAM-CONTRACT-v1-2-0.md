...
- **Security-critical set** — `hooks/secrets-gate*`, `hooks/spec-gate-commit*`, `hooks/dependency-gate*`, `hooks/test-gate*`, `hooks/eval-gate*`, `hooks/tdd-gate*`, `hooks/format-lint-gate*`, and `settings.json`. *`spec-gate-entry` is deliberately excluded: it is warn-tier by the protocol's own entry-warn/commit-block split; the block-tier `spec-gate-commit` is the critical one.*
...
5. `resolve_config` invariants are unchanged: `queue ⇒ loop | goal` (enforced at config, retrofit, and file levels — verified against `defaults.py`/`installer.py`); `prd_tier` enum (`micro|standard|full`) **[1.1.0 correction: "tier floors" was unverified against code — no floor rule exists in `defaults.py`; re-verify at rebase]**; archetype enum (`cli|library|service|fullstack|mobile|data-ml|ai-agent|platform|other`).
...
8. The security-critical AND autonomy-critical hook sets (§7.2) are present and match the contract lists.
...
- **SDK gate module (Bootstrap ≥ 2.1.0 substrate release; consumed via Tessera AC-PROTO-016 — pending the BR-03 fold).** *(The producing side's implementation plan calls this "Milestone B"; that label is non-normative and not relied on here.)* When the SDK-callable gate substrate lands (protocol IC-5, `gate_substrate: "sdk-callable"`, at the ≥ 2.1.0 substrate release), the enforcement layer is carried by a protocol-emitted, project-resident Python module. Locked constraints:
  ...
  - **API surface.** The module exposes **exactly one public builder** — proposed `build_hooks(config: dict) -> dict[str, list[HookMatcher]]` — taking the resolved project config and returning the Agent SDK hooks mapping (`{"PreToolUse": [HookMatcher(...), ...], ...}`). The exact symbol name and signature are locked at the substrate-release seam bump; until then this proposal is the working shape and any deviation is a seam-visible edit to this entry, not a quiet code choice.
  ...
  - **When implemented.** Entering the wire is a `seam_version` bump adding the builder API to §3 and `sdk_gates/gates.py` to the §7.2 security-critical set. Per the pin choreography, both land inside the **substrate-release seam MAJOR** (the `binds.bootstrap_protocol` re-point from the shell-conformant 2.0.0 release drops a previously-satisfying version — §8.1a — unless the owner pre-widened the range at the 2.0.0 pin, in which case these additions still force at least a MINOR).
