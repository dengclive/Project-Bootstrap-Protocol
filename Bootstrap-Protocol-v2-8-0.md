...
>
> **[2.8.0, was Unreleased post-2.7.4] The trigger, stated so a reader can check it
...
> The recipes below are the 2.5.0 recipes. What changed is that several gates this
> document already promised now actually enforce. Source:
> `docs/bootstrap-protocol-upstream-bugs-2026-07-28.md` — a **six-lens adversarial
> review of a real v2.5.0 install** (greenfield `fullstack`, all autonomous modes
> off, design steering + telemetry on, `gate_substrate: "shell"`) that **executed**
> the emitted hooks against crafted payloads instead of reading them. Static review
> of the same files had found neither the RCE nor the dead gates.
> **Why MINOR and not PATCH:** the emitted gates change *behavior*, not merely
> bytes. `test-gate`, `ci-mirror` and `format-lint-gate` were emitted `async: true`
> and therefore could not block at all — an async hook's exit code cannot reject a
> tool call and its stderr is suppressed — so `test-gate` printed *"Commit blocked:
> tests failing"* and the commit went through. This document presents "implementation
> passes local gates" as gate 5 of 6; for those hooks that gate did not exist. They
> now carry explicit timeouts (600 s / 900 s / 120 s) and block. An operator
> upgrading from 2.5.0 gets **stricter** enforcement than 2.5.0 actually gave them,
> which is a behavior change they must be told about, not a silent repair.
> **Why MINOR and not MAJOR:** because the *normative surface did not change.* §6.C
> already said `test-gate` blocks and the CI mirror exits 2; the emission was in
> violation of this document, not conformant to an older contract. Bringing an
> implementation into conformance with an unchanged spec is not a breaking change,
> even though it is a behavior change for anyone whose workflow had come to depend on
> the violation. The one place 2.6.0 changes *documented* behavior rather than
> repairing conformance is `dependency-gate`'s matching (see the Companion's
> Migration notes) — additive coverage within an existing gate's stated policy, which
> is MINOR by the same discipline.
> **What this document got wrong — corrected here, not only in the emission:** §6.A
> recommended `async: true` for slow hooks without naming the exit-code consequence,
> and the §6.C hook list recommended it for the **CI mirror**, a gate whose entire
> purpose is to exit 2 on failure [P1-1]; the §6.C **secrets gate** and Phase 2.7
> both described `Read`/`Write`/`Edit` wiring only, while `secrets.md` told the
> operator those paths were blocked — a shell call walked straight past it [P0-2];
> the **test gate** was described as trusting a marker file that is gitignored and
> agent-writable [lens A F4]; the **format/lint gate** was described running a
> mutating formatter [P2-6]; and the **cost log** bullet promised token spend the
> `Stop` payload never carries [P3]. Those seven corrections were applied in place
> to `Bootstrap-Protocol-v2-5-0.md` on 2026-07-28, marked at each site, because no
> 2.6.0 document existed yet to carry them; they are **native text here** and the
> markers are retained as provenance.
> **Backlog A-1 is decided: fail-closed.** With neither `jq` nor `python3` present
> every gate used to fall through its `case` and allow, silently. Gates now fail
> **closed** with a reason; hooks that are advisory by design declare `FAIL_CLOSED=0`
> and degrade to a logged no-op instead. The retrofit guarantee that case existed
> for — a parser outage cannot fabricate the retrofit exemption — is unchanged and
> now stronger.
> **One default flips in this document:** `audio_enabled` in the §6.E config block
> goes `true` → `false`, because no emitted hook has ever invoked a player; the
> old default advertised a capability that does not exist [P3]. **Exactly one, and the
> block is not uniformly emission-accurate — read the convention before assuming it
> is.** `drift_tier3_enforced` three lines above fails the same "capability does not
> exist" test against the *emitted stub*, and deliberately stays `true`: this block
> specifies the values a complete implementation uses, and publishing a
> safety mechanism as default-off would be a worse error than the mismatch. The
> installer emits `false` there and says so at the edit site; the divergence is
> annotated in the block itself rather than resolved by flipping. `audio_enabled` is
> different in kind — no implementation of audio exists anywhere, so `false` hides
> nothing.
> **Substrate parity (P2-1/2/3):** where `gates.py` and the shell suite disagreed,
> **the shell suite is canonical** — it is the default substrate and ships on every
> install. The SDK module implements **seven** gates (`secrets-gate`,
> `spec-gate-commit`, `dependency-gate`, `test-gate`, `eval-gate`, `tdd-gate`,
> `format-lint-gate`); an emitted `gates.py` declares whichever of those seven the
> config actually resolved, so a `fullstack` install with no eval gate declares
> six. It is a consistent *subset*: it must neither allow what the
> shell blocks nor block what the shell allows, and it is inert unless
> `gate_substrate: "sdk-callable"`. The shell suite additionally emits
> `spec-gate-entry`, `ci-mirror`, the drift detector and the session/alarm hooks,
> for which the SDK module has no equivalent — so under SDK dispatch **no gate runs
> the CI checks before a push**. Be precise about the scope of that gap: `push` is not
> categorically unguarded, because on an archetype that resolves `eval-gate` the SDK
> module does declare it on `PreToolUse`/`Bash` and does block a push touching prompt
> files. What is missing is the CI mirror specifically. That is a documented gap in
> the subset, not a parity claim.
> **Not protocol surface, deliberately excluded:** this repository's own
...
> was added or removed), no §7.3 provenance-marker or synthesize-file-contract change,
> no §7.4 shared sentinel
> change, no CLI entry point or contract-level flag, no §4.1/§5 table change, no
> `binds` change. The new `permissions.deny` key lives inside `settings.json`, which is
> already a §7.2 member, and is not a §4.1/§5 table or a sentinel.
> §8.4's own closing line governs — *"changes that touch only gate
> internals or dispatch policy do not bump `seam_version`."* `seam_version` stays
> 2.0.0 and commit-pinned consumers need no re-pin. The owed substrate-release seam
> re-cut (per the v2.2.0 head) is untouched and still owed; the Tessera pin is
> unaffected. See the Companion's Migration notes (v2.6.0 entry).

> **v2.5.0 — design steering, MINOR (2026-07-21).** One additive, opt-in
> recipe, applied changelog-first. No gate semantics change, no wire change, no
> state-schema change beyond one opt-in flag — the same additive, off-by-default
> shape as the v2.4.0 TEL-01 observability addition.
> **DS-01 (design steering, add-and-document):** a new skippable Phase 0
> decision `design_steering_enabled` (**default off; opt-in; operator-enabled
> during the interview**, same shape as `telemetry_export_enabled`), recorded in
> `.bootstrap-state.json`. When enabled, the wizard emits
> `.claude/steering/design.md` — a committed, operator-facing steering doc (under
> the existing "all steering docs committed" rule) carrying the UI/UX design
> invariants and the HONEST-USE-ONLY persuasion/pricing rules, self-contained for
> agents. When enabled, the wizard also offers an **optional advisory**
> `design-review` skill that reads `design.md` at the code-review gate (flags,
> never blocks). **compose-do-not-fork:** the persuasion rules' project-wide
> *ranking* authority is **not** duplicated here — `principles.md` (Phase 4)
> remains the sole ranking surface; `design.md` binds behavior within design
> scope and offers a *promotable* principle rather than asserting a second rank.
> Surfaced in Phase 0 step 6 (skippable-phase decisions), the Phase 0.5 preview,
> the Skip Policy, Phase 2's archetype question lists, Phase 7's optional-skills
> list, and Phase 8's `CLAUDE.md` steering-pointer list; recorded as a
> `.bootstrap-state.json` field. **Accessibility scope:** `design.md` carries an
> accessibility *invariant* (a design-time floor); accessibility **audit** and
> compliance frameworks (WCAG/ADA) stay out of scope per the Companion's
> unchanged exclusion. **Seam impact: none.** The sole delta is a
> workspace-resident steering artifact plus one opt-in state field and
> wizard/doc text; no wire surface, no gate, no `binds` re-cut. The owed
> substrate-release seam re-cut (per the v2.2.0 head) is untouched and still
> owed; the Tessera pin is unaffected. See the Companion's Migration notes
> (v2.5.0 entry).

> **v2.5.0 in-version corrections — DS-01 delta adversarial review (2026-07-21, applied per the in-version clarifying-fix precedent, no version bump).** Three findings from the two-layer delta review of the DS-01 fold, applied without a version bump and marked `[DELTA-nn]` at each edit site (same discipline as the TAR round below). The delta was found **sound as applied** — no fork, no stealth-MAJOR, no gate that harms a project it offers to, no unmeetable accessibility obligation — with three clarifying edits. **DELTA-01 (should-fix, owner-approved reopen of DR-2)** — the emitted `design.md` body's persuasion section read "(ranked highest)", the one place the compose-do-not-fork boundary was fragile: an agent reading `design.md` in isolation could over-read it as a *global* rank rather than highest-within-design-scope. Hardened at the artifact (not just the protocol prose): header changed to "(highest within this doc)" plus an appended sentence deferring cross-domain ties to `principles.md` as the sole ranking surface. Ripples into the implementation prompt — the flag-on golden fixture must be generated against the revised body (default-path digests unaffected; design steering is off by default). **DELTA-02 (nice-to-have)** — the archetype-gated offer excludes Service and Data/ML, but a headless-classified project that in fact ships a UI (admin panel, dashboard, docs/status site) had no signposted path; documented at Phase 0 step 6 that such a project can hand-set the flag or re-run as Full-stack/Platform (offer gated, flag not). **DELTA-03 (should-fix)** — the advisory `design-review` skill's residual was under-labeled against the delta's own stated threat model (LLM-generated UIs reproducing dark patterns at scale, EU Digital Fairness Act incoming); added an explicit honest-scope clause at the skill introduction stating it is a design-time floor and advisory flag, not a compliance control, and no substitute for legal review. Path C (hook enforcement) stays rejected; the clause *reinforces* the advisory-only decision by labeling its ceiling honestly. **Seam impact: none** (documentation and one workspace-resident steering-artifact body only; no wire surface, no gate, no `binds` re-cut; the owed substrate-release seam re-cut is untouched and still owed; the Tessera pin is unaffected).
...
> **Companion reference:** `Bootstrap-Protocol-Companion-v2-8-0.md` holds the Mental Model, Model Assignment Strategy, Post-bootstrap milestones, scope-exclusion list, Cheat Sheet, Glossary, Migration notes, and the rationale for phase numbering. Consult it when this file references those topics.
...
The protocol writes a state file at `.claude/.bootstrap-state.json` after each phase exit. It contains: archetype, completed phases, last phase started, PRD path, deferred items, synthetic profile (for "Other"), CI/CD opt-out flag, `bootstrap_protocol_version`, the three autonomous-mode opt-in flags (`loop_mode_enabled`, `goal_supervised_mode_enabled`, `queue_mode_enabled`), the `telemetry_export_enabled` opt-in flag (default `false`; when `true`, `.claude/steering/telemetry.md` is emitted — see the skip policy and Phase 0 step 6), the `design_steering_enabled` opt-in flag (default `false`; when `true`, `.claude/steering/design.md` is emitted and the optional `design-review` skill is offered — see the skip policy and Phase 0 step 6), and the three per-mode tracking lists:
...
For state-file migration from older protocol versions, see `Bootstrap-Protocol-Companion-v2-8-0.md`'s **Migration notes** section.
...
**State file naming convention:** all bootstrap-internal state files use the dotfile convention (`.bootstrap-state.json`, `.bootstrap-incomplete`, `.drift-ack`, `.drift-state`). Operator-facing artifacts use regular naming (`CLAUDE.md`, `Bootstrap-Protocol-v2-8-0.md`, etc.). This makes hidden state easy to gitignore as a group.
...
**The wizard surfaces due entries fail-loud, non-blocking.** When the operator changes a pinned model tier or the runtime floor moves (the same event that the v2.0.0 model remap represents, and that any later regenerate-config flow triggers), the wizard reads the ledger and surfaces every row whose re-validation trigger matches as a plain notice — it does not block the change, and it does not silently proceed. The point is only to make the calibration debt visible at the moment it becomes payable; the operator decides whether to re-tune. **[LIT-06] converts the notice into a measurement:** on any pinned-model or runtime-floor trigger, the operator re-runs the committed golden set (Phase 9 step 9) through each enabled autonomous mode and compares the [LIT-08] metrics against the stored baseline. (Surfacing remains a manual convention until the deferred wizard surfacing lands; the golden run is likewise operator-run, not automatic.)
...
The ledger **links** to the sections that own each default (§6.E, *Drift detector specifics*, for drift thresholds; Phase 9.5 for max-iterations and the token-multiplier note; Phase 7 step 5 for the preservation invariant [LIT-01]; Phase 9.5 *Max-iterations safety* for the cap redaction [LIT-07]) rather than restating their values as a second authority; the owning section stays the source of truth. Adding a row is the right move whenever a new default is introduced because a current model needs it — that is what keeps the staleness class from going invisible again.
...
6. **Surface skippable-phase decisions.** For each phase marked skippable in the Skip Policy, ask the operator: "Run / skip / decide later?" The phases and their defaults:
   ...
   - **Design steering (`design_steering_enabled`)** — **default skip** (opt-in only — independent of every autonomous mode; enabling it emits `.claude/steering/design.md`, offers the optional `design-review` skill, and records two state flags (`design_steering_enabled` plus the gated `design_review_skill_enabled`), and changes no gate, wire, or runtime behavior). The wizard offers it for user-facing archetypes (Full-stack, Mobile, AI/agent, Platform, and user-facing "Other"); the operator opts in. A headless-classified project (Service, Data/ML) that in fact ships a user-facing surface — an admin panel, dashboard, or docs/status site — can still enable design steering by hand-setting `design_steering_enabled: true` in the ANSWERS block or by re-running the wizard as Full-stack/Platform; the offer is archetype-gated, but the flag itself is accepted for any archetype (the wizard does not reject it). [DELTA-02]
   ...
   **State file recording:** Record decisions in `.bootstrap-state.json`. The 9.5 decision is also written as the top-level `loop_mode_enabled` flag; the 9.6 decision is written as `goal_supervised_mode_enabled`; the 9.7 decision is written as `queue_mode_enabled`; the telemetry decision is written as the top-level `telemetry_export_enabled` flag (default `false`); the design-steering decision is written as the top-level `design_steering_enabled` flag (default `false`), and the optional-skill sub-decision as the top-level `design_review_skill_enabled` flag (default `false`; only ever `true` when design steering is enabled — it is gated on the primary at write time so the recorded skill flag can never disagree with what was emitted); the 7.6 decision is recorded under `skippable_phase_decisions.phase_7_6_spec_roster`. These decisions feed Phase 0.5's preview so the operator sees the actual scope before committing.
...
- **Full-stack app:** all of the service questions plus frontend stack, state management, styling approach, accessibility baseline, error monitoring tool, and — when `design_steering_enabled` — design conventions (offer to generate `.claude/steering/design.md`; the operator's answers here fill its Project-specifics block: styling approach, component library, accessibility baseline, brand/shadow rules).
- **Mobile app:** platform versions supported, build signing, native module policy, offline behavior, crash reporting tool, and — when `design_steering_enabled` — design conventions (offer to generate `design.md`; answers fill its Project-specifics block).
...
- **AI / agent system:** model provider(s), prompt versioning, context budget per call, eval harness location, cost tracking mechanism, fallback behavior, prompt + output logging, and — when `design_steering_enabled` (i.e. the agent system has a user-facing surface) — design conventions (offer to generate `design.md`).
- **Other:** the AI uses the synthetic profile from Phase 0 to pick the most relevant subset of the above and asks accordingly, and — when `design_steering_enabled` (the operator opted in; the wizard offers it when the synthetic profile's distribution dimension is user-facing — hosted service / app store / browser store — or for Platform archetypes, per user-facing unit) — includes design conventions in that subset and offers to generate `design.md`.
...
4. **[W-1] Always ask where the project's commands execute.** This is not a stylistic detail — it decides whether Phase 7 may emit `isolation: worktree`, and getting it wrong produces a verification gate that reports green on code it never compiled. Ask it of every archetype; containerized dev environments are not archetype-specific, which is exactly why this pairing went unnoticed until it was reported.

   **Question phrasing for command execution location (use verbatim):** *"Do your test, lint, format, typecheck and CI commands run in the directory they are invoked from? Answer yes for anything that runs locally (`pytest -q`, `npm test`, `make ci`) and for container invocations that follow the caller (`docker run -v "$(pwd)":/app ...`). Answer NO if a command reaches its code through a fixed mount — `docker compose exec`, `kubectl exec`, `ssh`, `vagrant ssh`, a devcontainer CLI — because those land in a tree the mount chose, not the one you were standing in. This matters because the implementer subagent works in its own git worktree: if the gates cannot follow it there, they compile and test the main checkout instead, and report green on code that was never built. Answering no makes the installer drop the worktree isolation rather than leave you with a gate that lies."*

   The answer becomes `commands.execute_in_cwd` in `bootstrap.config.yaml`, default `true`. **Never infer it from the repo** — a `docker-compose.yml` in the tree is not evidence that the test command goes through it, and inferring wrong in the permissive direction re-arms the fail-open. If the answer is no, say plainly what the installer will do about it (drop `isolation: worktree`, start `max_concurrent_tasks` at 1) rather than only recording the flag.
5. Write `.claude/steering/tech.md` with explicit `## Do Not Touch` and `## Documentation Policy` sections, and an **Execution location** line under the command table stating the answer and its consequence.
6. Show and ask: **approve / edit / start over**.

**Exit criteria:** `tech.md` exists. The "do not touch" list is explicit and approved. The command-execution-location answer is recorded — including when it is the default `true`, so a later reader can tell "asked and answered yes" from "never asked".
...
2. Write `.claude/steering/secrets.md` with: never-read paths, **which tool surfaces the gate covers** (the file tools *and* `Bash` arguments — see step 3), secret-detection patterns, AI behavior on encounter, rotation policy summary, and **what to do when the gate refuses a search the operator believes is safe**. **[X-31]** That last section is not padding: the gate matches the path a command *names*, so a negated glob (`rg -g '!*.pem'`) is refused, and the operator needs the positive-scope answer (`rg -g '*.md'`) in the same place the never-read list lives. `secrets.md` and the gate's refusal message state one rule and are changed together.
3. Add the never-read paths to the **secrets gate** hook (separate from spec gate per the v1.2.0 split). The secrets gate blocks `Read`/`Write`/`Edit`/`NotebookEdit`/`Grep`/`Glob` on these paths in Phase 6, **and scans `Bash` command arguments for them** — a shell call was the access route the file-tool wiring left open. **[Corrected 2026-07-28, upstream P0-2.]**
4. Show and ask: **approve / edit / start over**.
...
> - **Pin a minimum Claude Code runtime** (≥ v2.1.210: earlier versions stall an unattended session on hook timeout). This pin belongs in the seam `binds` set. **[Corrected 2026-08-13, X-51.]** This bullet used to justify the pin as *"for fail-closed `PreToolUse` timeout behavior"*. **No runtime delivers that** — measured live at v2.1.227, a hook cancelled at its declared `timeout` exits 124/137/143 and the tool call proceeds, because only exit 2 blocks. The pin is KEPT on its other, true ground (no unattended stall); the fail-closed rationale is retracted. See the Secrets-gate entry below.
>
...
- **Use `async: true` for slow hooks that do not need to block, and `timeout` for slow hooks that do.** Hooks block tool execution by default. For checks that take more than a second or two (full lint suites, type-checkers on large codebases, security scans), `async: true` runs them in the background. Released January 2026. **[Corrected 2026-07-28, upstream P1-1]** An async hook's **exit code cannot block the tool call**, and its stderr is suppressed by default — so a hook that exits 2 to block, while marked async, is silently inert. `async: true` is therefore only correct for hooks that are purely informational. A slow hook that must be able to block takes an explicit `timeout` (seconds) instead. v2.5.0's §6.C hook list recommended `async: true` for the CI mirror, a gate whose entire purpose is to exit 2 on failure; that recommendation was self-contradictory and is corrected in the CI-mirror bullet of §6.C below.
...
2. Propose the standard set, with archetype-specific additions (for "Other" archetype, hook selection is driven by the synthetic profile's `phase_overrides.phase_6_hooks` array set in Phase 0; the AI proposes those plus any defaults marked "all"):
   ...
   - **Secrets gate** (all) — `PreToolUse` on `Read|Write|Edit|NotebookEdit|Grep|Glob` **and on `Bash`**. Blocks any path in `secrets.md` never-read list. Distinct hook from the spec gate (not "split"); kept as its own item because secrets exposure is catastrophic and the model should fail loudly rather than continue. This *does* block mid-plan, which is the deliberate exception to the "don't block mid-plan" rule above. **[Corrected 2026-07-28, upstream P0-2.]** The Read/Write/Edit-only wiring this line used to describe left every never-read path reachable through a shell call while `secrets.md` told the operator otherwise. On the `Bash` surface the command is tokenized the way a shell would — split on *unquoted* whitespace, quoted runs joined into one argument — so `cat .env` blocks while `git commit -m "fix the .env loader"` does not; the mid-plan exception is for secrets, not for prose. One deliberate carve-out: the conventional dotenv **template** names (`.env.example`, `.env.sample`, `.env.template`, `.env.dist`, `.env.defaults`) are exact-matched on the **basename** and allowed — never as a prefix or suffix form, so `.env.example.real` still blocks. This gate also carries an explicit **60 s** `PreToolUse` timeout: it is the hottest matcher in a session (every `Bash` call as well as every file tool) and its pure-bash tokenizer is superlinear in command length, so an unusually long command has no cost bound of its own. **[Corrected 2026-08-13, X-51/X-52 — and this correction inverts the security posture, so read it before relying on any timeout.]** This line used to read *"A `PreToolUse` timeout fails **closed** at the seam's runtime floor, so the bound refuses a pathological command rather than allowing it."* **That is backwards.** Only exit code **2** blocks a `PreToolUse` call; a hook cancelled at its declared `timeout` is killed and exits 124/137/143 — never 2 — so the call **PROCEEDS** and the gate fails **OPEN**. Verified live against Claude Code, not inferred: a hook that sleeps past `timeout: 2` and would then `exit 2` is killed and the command runs; the identical hook allowed to finish blocks it. `FAIL_CLOSED=1` does not rescue it — that governs what the hook does on *its own* error and says nothing about the harness killing it from outside. **The declared timeout is therefore the attacker's budget, not a safety net.** A correct-but-slow gate is a bypass: only two postures are actually sound — deny cheaply *before* the expensive work, or be provably fast on every admissible input. Tracked as X-51 and X-52 (both `done`). The open residuals — X-53 … X-57 — were measured against `dependency-gate`, not this gate; the *mechanism* is shared by every `PreToolUse` hook, the measurements are not this gate's. **[X-31] The gate matches the PATH the command names, not the operator's intent, and the refusal must say so.** A *negated* glob still names the protected path, so `rg -g '!*.pem' TODO` is refused even though it reads strictly fewer files than the bare `rg` this gate allows — reading that negation as safe means reproducing the search tool's own flag grammar exactly, which is a parser, not a gate condition, and four rounds of trying it produced 4, then 6, then 12, then ~20 blocking fail-opens before the attempt was abandoned (see `docs/agentic-harness-security-kb.md` §4.9). The over-refusal is therefore **kept**, and the refusal message carries the workaround instead: **scope positively** — name what you *do* want (`rg -g '*.md' TODO`), which excludes the protected paths structurally with nothing left to parse. The emitted `secrets.md` states the same rule, and the two must be changed together.
   - **Test gate** (all) — `PreToolUse` on git commit. Runs the project's test command and blocks unless it passes. **[Corrected 2026-07-28, lens A F4.]** It used to skip the run when a marker file said the tests had passed since the last source edit; that marker is gitignored and agent-writable and no gate protects it, so `touch .claude/.last-test-pass` disabled the gate. A gate must not trust an input the agent can write, and verifying the marker's *contents* is no better — whatever the gate can compute, the agent can compute too — so there is no marker: the tests run on every commit attempt, bounded by the hook's 600 s timeout, exactly as the CI mirror runs on every push.
   - **Format/lint gate** (all) — `PostToolUse` on Write/Edit. Runs the configured **lint/check** command (never a mutating formatter — see below) and reports; non-blocking feedback. Emitted with an explicit `timeout` rather than `async: true`: async suppresses hook stderr, which is this hook's entire output, so an async lint gate reports to nobody. **[Corrected 2026-07-28, upstream P1-1/P2-6.]**
   - **CI mirror** (all with CI) — `PreToolUse` on git push. Runs the same checks CI runs locally, and exits 2 to block the push when they fail. Emitted with an explicit `timeout` (900 s default), **never** `async: true` — an async hook cannot block, so an async CI mirror is a no-op that prints nothing. **[Corrected 2026-07-28, upstream P1-1.]**
   - **Session-event log** (all) — session-end record, not per-call. Appends one `{"event","session_id","ts"}` line to `.claude/logs/session-events.jsonl`. **[Corrected 2026-07-28, upstream P3.]** This bullet used to name `cost.jsonl` and claim task ID, total token spend and tool-call count; the `Stop` payload carries no token or dollar figure, so the hook never had one to record and the file recorded none. Cost belongs to the telemetry surface — `.claude/steering/telemetry.md` documents Claude Code's OTel `token.usage` by `agent.name` against an operator-owned backend.
   ...
   - **Dependency gate** (all) — `PreToolUse` on `Bash` calls matching package-install patterns. Blocks unless every package named on the command line already appears on the approved list in `.claude/steering/deps.md`. **[Corrected 2026-08-03, issue-batch #30–#33 documentation audit.]** This line used to promise a second escape — *"or the operator confirms in-session (which then prompts an update to `deps.md`)"* — and no such escape exists, or can. A `PreToolUse` deny is evaluated before the call runs and there is no in-session channel that can wave one through; the emitted hook reads `deps.md` and nothing else. The real sequence is: the gate denies, the operator's approval is **written into `deps.md` in the same change**, and the retried command passes. That is what the refusal says, and it is what the emitted `deps.md` says. **The approved list is also not the only reason this gate refuses**, and the other reasons no list entry can lift are enumerated in the emitted `deps.md` under *"What this list does not cover"*: a package-index or remote-source override (`--index-url`, `PIP_INDEX_URL=…`, `-f <url>`), a requirements/constraints **file** whose contents the gate cannot read, the run-without-installing channel (`npx`, `uvx`, `pnpm dlx`, `pipx run`, `uv run --with`), **downloaded bytes piped into an interpreter** (`curl … | sh`, `curl … | python3 -c …`), and a downloaded file that the same command then runs. Each prints its own two-line reason naming which one fired — an operator who reads only this bullet will look for a package name to approve and find none.
...
- ✅ Hooks log to `.claude/logs/hooks.log` for audit; logs do not include sensitive content. The log **rotates** (1 MiB default) — an audit log with no rotation grows without bound on a busy repo and eventually becomes the reason the operator disables hooks. **[2.6.0, upstream P3.]**
- ✅ Exit codes follow convention (0 / 1 / 2 — see below). Note the asymmetry that makes exit 1 dangerous in a gate: exit 2 blocks, but exit 1 is *"hook error, tool proceeds"*. A gate that dies under `set -e` — a failed `mkdir`, a failed `mktemp` — therefore **allows** the call it was written to deny. Keep incidental work (logging, temp files) non-fatal, and route any unexpected failure through the deny path with an `ERR` trap. **[2.6.0, upstream P0-3c.]**
- ✅ **Fails closed when the parser does not WORK — which is not the same as missing.** A gate that cannot parse its payload must deny with a reason — never fall through to allow. With neither `jq` nor `python3` present, every gate used to fall out of its `case` and permit the call, silently. **Absence is the easy half.** A parser that is *present and broken* — a version-manager shim with no version resolved, a binary whose dynamic link is broken (`libonig.so.5`), a non-executable file of the right name — satisfies `command -v` and then fails, and a selector written as `if have_jq … elif have_py` binds its fallback to jq's *absence* and never reaches a healthy `python3` on the same PATH. Choose the parser on **failure**, not on presence: try each in turn, accept its output only if it exited clean, and never let `|| true` erase the difference between "returned nothing" and "could not run". Both unusable must reach the same deny as neither installed. **[2.6.0, upstream P0-3a–c; the present-but-broken selector is P0-3d, fixed post-tag 2026-07-31 — see `docs/agentic-harness-security-kb.md` §4.6.]** Hooks that are *advisory by design* declare `FAIL_CLOSED=0` and degrade to a logged no-op instead; everything that exists to block declares nothing and denies. In the emitted set that is six hooks — the drift detector, the format/lint reporter, the session-event log, the two alarms, and **`spec-gate-entry`**, which is a gate by name but warn-tier by the entry-warn/commit-block split. The last one is the trap worth naming: a fail-closed `UserPromptSubmit` hook blocks the *operator's own prompt*, and on a `Stop` hook exit 2 means "do not stop". Classify by whether the hook exists to block, never by whether "gate" is in its name. **[2.6.0, upstream P0-3; this decides backlog A-1.]**
- ✅ **No arithmetic over unvalidated file or JSON input.** Bash performs command substitution *inside* arithmetic evaluation, so `n=$(( $(cat "$STATE") + 1 ))` against an agent-writable state file is arbitrary command execution — `PATH[$(touch /tmp/PWNED)]` in that file runs on the next hook fire, bypassing every `PreToolUse` `Bash` gate. Read the value, validate it as an unsigned integer, then add. Audit the whole class, not the one instance. **[2.6.0, upstream P0-1 — CONFIRMED RCE.]**
- ✅ **Never trust an input the agent can write.** A gitignored, agent-writable marker file (`.claude/.last-test-pass`) is not evidence that tests passed — `touch` disables the gate. Verifying the marker's *contents* is no better: whatever the gate can compute, the agent can compute too. Re-run the real check, bounded by the hook's `timeout`. **[2.6.0, lens A F4.]**
- ✅ **Payload size has a ceiling on the environment, not on stdin.** Passing a whole tool payload in an environment variable caps at 128 KiB on Linux, so a large `Write` fails `exec` — and `|| true` turns that failure into an allow, while the same payload with `jq` present blocks. Give the parser the payload on **stdin**. **[2.6.0, upstream P0-3a.]**
...
- ✅ **A JSON parser that actually RUNS** — most hooks parse JSON input from Claude Code. Do **not** gate this on `command -v jq`: that answers *"is there a file named jq on PATH"*, which is not the question, and it reports success for a shim, a broken dynamic link and a non-executable file alike. Try `jq`, and fall back to Python's `json` module whenever jq **fails**, not only when it is absent. Verify by executing a known literal through the emitted hook with a *broken* jq on PATH, not merely with jq removed.
...
- ✅ **Verb matching anchors to command position, not substring.** A fixed-spacing literal like `git commit` misses `git  commit` (two spaces), a tab, `git --no-pager commit` and `git -C /repo commit`, while firing on `git commit` inside a comment, a quoted string or a grep pattern. Normalize whitespace and test the verb in command position. **Command position includes an invoker's argument.** `sh -c "git commit"`, `bash -c`, `ssh host "…"` and their siblings take a *command line*, not data, so the quoted argument opens a fresh command position and must be re-segmented and re-tested — to a stated nesting depth, because `bash -c "sh -c \"git commit\""` is reachable. Anchoring without this rule is the one regression that looks like conformance: it passes every false-positive test above while quietly dropping `sh -c`. The upstream report proposed accepting that loss; **do not** — accepting it also splits the shell and SDK substrates, and the allow side is the fail-open side. Handle backslash escapes before the quote walk, or the scanner desyncs and the nesting is silently lost. **[2.6.0, upstream P1-4; invoker rule and depth from the round-3/round-4 remediation, which supersedes the report's accepted trade-off — this closes backlog J-1.]**
- ✅ **Session-scoped state keys on a session ID the runtime actually provides.** `CLAUDE_SESSION_ID` is *not* exported to hooks; a counter keyed on it collapses every session into one shared `.drift-state-default` that never resets — observed at 274 against a threshold of 50, firing on every tool call forever. Read `.session_id` from the hook payload and sanitise it for path safety before using it in a filename. **[2.6.0, upstream P2-7.]**
- ✅ **Paths are absolute, and "tool missing" is distinguished from "check failed".** A relative `find src` evaluated against an absolute marker means a project with no `src/` passes every commit with no test run, forever, once the marker exists. Search absolute paths across the project's real source roots, and treat exit 127 (toolchain absent) as a distinct outcome from a genuine failure rather than reporting "tests failing" for both. **[2.6.0, upstream P2-5.]**
- ✅ **Path patterns have no implicit leading wildcard.** Treating `.env*` as `*.env*` hard-blocks `src/my.envelope.gleam` and `docs/dev.environment.md` mid-plan. Match dot-segment aware, so `config.env` and `prod.env` still block while word-interior matches do not. **[2.6.0, upstream P2-4.]**
- ✅ **Prove the gate can fire at all.** Three emitted gates shipped that *could not* block: two were `async`, and one guarded its only warning behind `[ ! -s INDEX.md ]` when `INDEX.md` is always emitted non-empty — dead code that never once fired. A fourth blocked *every possible* first commit. Emission determinism cannot catch any of this; only execution can. Write the gate a payload that **must** be denied and a payload that **must** be allowed, and assert both exit codes. **[2.6.0, upstream P1-1/P1-2/P2-8.]**
...
**Session identity:** the session ID is read from the **hook payload's `.session_id`** and sanitised for path safety before use — it is *not* available as an environment variable, and a hook that reaches for `CLAUDE_SESSION_ID` gets nothing and silently collapses every session into one shared state file **[2.6.0, upstream P2-7]**. It is stored at `.claude/sessions/.session-<id>` with start timestamp. State files for all alert categories (`.drift-ack-<id>`, `.drift-state-<id>`, `.drift-tier3-<id>`, `.decision-pending-<id>`, `.quiet-<id>`) are namespaced by session ID. On `/clear` or new conversation start, a new session ID is generated and old state files older than 7 days are purged.
...
> **[X-30] The alarm hook must NEVER truncate that file.** This paragraph tells the agent to write four fields into `.decision-pending-<session-id>`; the emitted hook must therefore create it if absent and refresh its mtime, and must not blank it. Through 2.6.1 the hook ran an unconditional `: >"$P"` on every fire, so a conforming agent and a conforming hook **could not both be right** — the four fields were erased at exactly the moment the halt exists to deliver them, and every `.decision-pending-*` file on a real install measured 0 bytes. What is load-bearing about this file is its **existence** (the alarm cue) and its **mtime** (the 7-day sweep in Phase 7 step 6); nothing in the emitted tree reads its contents or keys on its emptiness, so blanking it bought nothing and cost the payload. An implementation that needs fire-once-per-escalation semantics must get them from something other than truncating the operator's only record. See also the loop-halt contract in Phase 9.5, which promises the operator "the decision-pending file **ready for action**".
...
drift_tier3_enforced=true                      # tier 3 hard-blocks tool calls until /clear
                                               # NOTE [2.6.0]: this is the value for a
                                               # COMPLETE implementation, and it stays
                                               # `true` because defaulting a safety
                                               # mechanism to off is the wrong default to
                                               # publish. The deterministic installer
                                               # emits `false` here instead, because the
                                               # hooks it emits implement the tier-1
                                               # notice only — see "Honest scope" in the
                                               # emitted file and backlog I-1. Contrast
                                               # `audio_enabled` below, which is `false`
                                               # in BOTH: no implementation of it exists
                                               # anywhere, so there is no capability the
                                               # default could be hiding.
...
- **Worktree isolation for subagents** — will be wired into the agent definitions in Phase 7. Each subagent invocation gets its own git checkout; clean worktrees auto-clean. Phase 7 will set `isolation: worktree` in the `implementer` agent's frontmatter (and others where applicable) **when the Phase 2 command-execution answer permits it** — see the compatibility requirement two bullets below; it is not unconditional. Mentioned here so the operator understands the drift-prevention strategy before MCP decisions.
  ...
  - **[W-1] Worktree isolation requires that the gate commands follow the working directory.** This is a compatibility requirement between two protocol features that do not automatically compose, and it is load-bearing. The emitted gates run the Phase 2 command **bare** — no `cd`, no `$CLAUDE_PROJECT_DIR` — and that is the correct design: a plain `pytest -q` or `npm test` inherits the hook process's cwd, which inside a worktree *is* the worktree. **Plain commands compose fine.** What breaks the pairing is **fixed-mount indirection**: `docker compose exec -T app pytest` enters a container whose bind mount points at the main checkout, so cwd is irrelevant and the command always runs against `/app`. Same shape for `kubectl exec`, `ssh`, `vagrant ssh`, and devcontainer CLIs. (`docker run -v "$(pwd)":/app …` *does* follow cwd and is fine — it is specifically the fixed mount.) Under that pairing an `implementer` works in `.claude/worktrees/<n>/` while its gate compiles and tests the main checkout: **the gate passes, and the code the agent wrote was never built** — a fail-open in a verification control, of the same class as the parser outage A-1 decided fail-closed. Phase 2 therefore asks the question outright and `commands.execute_in_cwd` records the answer; Phase 7 emits `isolation: worktree` only when it holds. **Confirm it rather than assume it:** from inside `.claude/worktrees/<n>/`, run your test command's transport with `pwd` substituted (`docker compose exec -T app pwd`). If the path printed is not the worktree, the commands do not honor cwd.
    - **The load-bearing assumption, stated as one:** that Claude Code sets an `isolation: worktree` subagent's working directory to its worktree. That is the documented behavior and everything above rests on it. If it were ever untrue, the pairing would fail more broadly rather than less — every command, not just fixed-mount ones, would run against the main tree — so the mitigation above is directionally right either way. The `pwd` probe settles it for a given environment in one command, which is worth more than a general claim.
...
**Lexical-first retrieval guardrail [LIT-04] (normative).** Exact recall — which files reference a spec, which decisions touched X, secret-path matching — must use lexical or pointer retrieval (grep/ripgrep, `.claude/specs/INDEX.md`), never single-vector semantic search alone. Claude Context remains recommended for fuzzy code discovery on large codebases, paired with a lexical fallback. A vector-backed memory MCP, if ever installed, is a fuzzy supplement — never the system of record for governance facts. If a vector layer is ever chosen, prefer late-interaction/multi-vector or hybrid backends over single-vector. (An operational rule for governance-fact recall — it survives under both the embedding-ceiling result and its rebuttal; the protocol already leaned this way, and the rule makes the lean stated.)

**The tool harness shapes the memory store as strongly as the model does [LIT-05].** Changing search or edit tooling is a store-affecting decision, not a neutral convenience — record it the way a model change is recorded. (See-also, same composition class: **[W-1]** above — harness components composing non-neutrally, there as gate fail-open rather than store shape.)
...
1. Create skills (each is a `SKILL.md` file under `.claude/skills/<skill-name>/`). **Model assignment per skill:** skills are invoked from a session, so the model used is whatever the calling session is using. To enforce per-skill models, include a recommendation in the skill's frontmatter or body (e.g., "Recommended: invoke from an Opus session for spec-review") so the operator or calling agent picks the right model. The Model Assignment Strategy table in `Bootstrap-Protocol-Companion-v2-8-0.md` is the source of truth for which skill belongs on which model.
   ...
   - `checkpoint` — generates a structured session synopsis and writes it to `.claude/sessions/<timestamp>-checkpoint.md`. **[X-33] `<timestamp>` comes from the clock, never from model memory — `date -u +%Y-%m-%dT%H%MZ`, output used verbatim** (model-supplied stamps have run ahead of true UTC; a wrong stamp poisons every consumer that sorts checkpoints by name). Synopsis covers: current task and spec ID, completed work this session, in-flight changes (uncommitted), files touched, key decisions made, open questions, state of tests, next steps. Used explicitly by the operator before `/clear`. Updates `INDEX.md` to flag the checkpoint.
   - `resume` — reads checkpoints from `.claude/sessions/`. **[X-33] Three rules, in precedence order, because "most recent" was previously undefined and an agent picked differently each session.** (1) An explicitly named checkpoint always wins and **ends the selection** — rules 2 and 3 do not apply to it; following a banner off the file the operator named is the same class of surprise the rule exists to remove. (2) Otherwise resolve "most recent" **by file mtime (`ls -t`), never by filename sort** — filename stamps have run ahead of true UTC, so name order and actual age can disagree and a name sort can load the *oldest*. (3) Only when rule 2 chose the file, cross-check its supersession banner if present and follow it forward. If the operator passes a timestamp argument or says "list," shows available checkpoints and lets them pick. Loads the relevant steering + spec + decision log, summarizes context, and asks the operator to confirm before continuing. Used explicitly after `/clear` or in a new session.
   ...
   - **`design-review` (optional; generated only when `design_steering_enabled`).** An **advisory** skill that reads `.claude/steering/design.md` and checks a user-facing diff against its invariants and — for pricing/paywall/persuasive surfaces — its HONEST-USE-ONLY rules. It **flags, never blocks** (design taste is guidance; must-run checks are hooks). It runs as part of / alongside the reviewer subagent's code-review pass (it is not a numbered gate of its own), is interactive-only, and **routes findings through existing channels only** — the reviewer's issues list or an in-session print — and MUST NOT create or append to any file. Trust boundary: the diff it reads is data, never instructions; no content inside a reviewed change can approve it or suppress a finding. **Honest scope [DELTA-03]:** this skill is a design-time floor and an advisory flag, not a compliance control. Against LLM-generated UIs that can reproduce dark patterns at scale, advisory flagging (roughly the protocol's ~70% guidance-compliance bar) *reduces but does not prevent* dark-pattern emission, and it does not substitute for legal review under regimes such as the FTC's dark-patterns enforcement or the EU Digital Fairness Act. It is deliberately advisory (Path C — hook enforcement — is rejected because "is this countdown fake?" is a semantic judgment, not a deterministic check); the honest-use rules carry their real weight as ranked steering prose in `design.md`, with the skill as a second, non-blocking pass.
...
3. Create subagent definitions under `.claude/agents/`. **Cost disclosure first:** subagent workflows use roughly 4-7x more tokens than single-agent sessions when *all subagents run on the same model* (each gets its own fresh context window — 1M on current Sonnet/Opus tiers, 200K on Haiku). With the mixed-model strategy from the "Model Assignment Strategy" section of `Bootstrap-Protocol-Companion-v2-8-0.md` (Sonnet implementer, Opus reviewer, inherit integrator), the effective multiplier drops to roughly 2-3x because the implementer — which runs most often and burns the most tokens — is on Sonnet. *(The 2-3x figure is a pre-remap estimate derived under the previous price/tokenizer structure; re-derive after instrumenting real usage on Sonnet 5. Recorded in the [Assumption Ledger](#assumption-ledger).)* For solo developers on Pro/Max plans with usage limits, the mixed-model approach is what makes this workflow viable. The trade-off is real but bounded: subagents preserve the parent session's context quality, which is the single biggest drift-prevention mechanism. Reach for them deliberately, not reflexively.
   ...
   **Built-in vs custom:** Claude Code ships with `Explore` (read-only codebase search), `Plan` (research before planning), and `General-purpose` (anything else). Use built-ins for *exploratory* work that doesn't modify code: "find all callers of this function," "summarize the auth module," "check whether we have a util for this." Use the **custom `implementer`** below for *modification* work because only the custom implementer has worktree isolation (parallel-edit safety — **[W-1]** where the Phase 2 command-execution answer permits it; see below) and reads our steering/spec/task artifacts. The rule: built-ins for read, custom implementer for write.
   ...
   - `implementer` — picks up a task file, reads steering + spec + task, implements, commits, logs decisions. Configuration:
     - **`model: sonnet`** (per Model Assignment Strategy in `Bootstrap-Protocol-Companion-v2-8-0.md` — Sonnet handles execution against a clear spec efficiently).
     - **`isolation: worktree`** — each invocation gets its own git checkout; clean worktrees auto-clean on no-change exits. **[W-1] Conditional on the Phase 2 command-execution answer.** The line is emitted only when `commands.execute_in_cwd` is `true`; when it is `false` the frontmatter carries no `isolation:` key at all, because a gate that runs somewhere other than where the agent worked reports green on code it never built (see Phase 6.5). The override is `workflow.implementer_isolation` — `auto` (default; derive from the commands answer), `worktree` (force on, correct when each worktree gets its own mount, e.g. a per-worktree compose project), or `none` (force off). **Set it in `bootstrap.config.yaml`, never by hand-editing the emitted `implementer.md`:** a hand-edit trips the installer's digest guard, after which that file is `SKIP`ped on every subsequent run and the operator keeps their local fix while silently losing every upstream fix to it.
     - **[W-1] Whichever way it resolves, the emitted agent body says so and why**, so the next reader can tell a deliberate absence from an oversight and does not "fix" the isolation back into a fail-open.
     - **[W-1] Isolation off costs parallelism, and the protocol pays it explicitly:** with every implementer in the main checkout, concurrent tasks would edit one tree, so `max_concurrent_tasks` is emitted as `1` rather than the usual `2` in both `backlog.md` and `auto-config.md`. That is the deliberate trade — a visible concurrency constraint in place of a silent green gate.
   ...
   - `reviewer` — reads diff against spec, returns issues. Configuration:
     - **`model: opus`** + **`effort: high`** (per Model Assignment Strategy in `Bootstrap-Protocol-Companion-v2-8-0.md` — code review is judgment-heavy and a missed issue can ship).
   ...
   - `integrator` — handles merge conflicts between parallel tasks. Configuration:
     - **`model: inherit`** (per Model Assignment Strategy in `Bootstrap-Protocol-Companion-v2-8-0.md` — match main session because conflict complexity varies). If main session is on Haiku for some reason, override to `model: sonnet`.
...
5. Initialize `.claude/learnings/` with a README explaining the convention (one file per learning, titled by topic, dated). **Note:** `learnings/` is for cross-feature insights; `decisions.md` is for within-feature choices; `sessions/` is for session-recovery state; **`progress.md` (below) is the within-task status + dead-ends ledger.** **Routing [LIT-03]:** verbatim artifacts (retained trajectories, raw checkpoints) serve strong consumers — the operator, forensic resume; distilled artifacts (`decisions.md`, `learnings/`, checkpoint synopses) serve weak or cheap consumers — Haiku-tier priming, autonomous re-priming; route by consumer strength (full statement in Phase 9.5). **Per-task progress artifact (`.claude/specs/<slug>/progress.md`):** created at task start and updated at every iteration boundary and at each interactive checkpoint write. Required sections: `Status` (one line), `Completed`, `In flight`, and `Failed approaches` (each entry records what was tried, why it failed, and a do-not-retry flag). It **links** to `decisions.md`, `learnings/`, and the latest checkpoint and MUST NOT duplicate their content — compose-do-not-fork applied to the protocol's own artifacts; a `progress.md` that inlines a `decisions.md` entry rather than linking it is a lint failure. Committed (operator-facing audit record). **Preservation invariant [LIT-01] (normative; applies to every memory artifact above — `learnings/`, `decisions.md`, `sessions/`, `progress.md`, checkpoint synopses — and to any consolidation pass, operator-run or auto-memory):** any rewrite or summarization of an existing memory artifact must preserve every load-bearing fact — decisions, failed approaches with their do-not-retry flags, source locators and citations — or link to it. Condensation may drop prose; it may never drop a fact, a flag, or a citation. Over-preservation bloat is bounded by the compose-do-not-fork lint above (link, don't inline) and by the [LIT-02] consolidation path (Phase 10) for deliberate size reduction. Recorded in the [Assumption Ledger](#assumption-ledger): the invariant compensates for current-generation condensation tendency; re-validate on model change.
6. Initialize `.claude/sessions/` with a README documenting the checkpoint schema and the audio alert state-file mechanisms.

   > **[X-33] `<timestamp>` is read from the clock, by EVERY producer, without exception.** `date -u +%Y-%m-%dT%H%MZ`, output used verbatim. This binds every path in this document that writes a checkpoint — the `/checkpoint` skill, the tier-3 drift demand (§6.E Phase A), the loop-mode and goal-supervised implementer variants, and the `CLAUDE.md` loop addendum — not only the interactive one. **The unattended paths are the ones that matter most:** a stamp supplied from model memory has run ahead of true UTC in practice, and `/resume` then loads by mtime while a human reading the directory sorts by name, so the two disagree with nobody watching. State the rule wherever an agent is told to write a checkpoint; a write path that omits it reproduces the defect in the path with the least supervision.

   The directory holds: `<timestamp>-checkpoint.md` files, plus per-session state files for the audio alert system (`.drift-ack-<id>`, `.drift-state-<id>`, `.drift-tier3-<id>`, `.decision-pending-<id>`, `.quiet-<id>`, `.session-<id>`). When loop mode is opted into at Phase 9.5, this directory also holds per-task loop state: `decisions-log-<task-id>.md`, `loop-final-<task-id>.md`, `.loop-active-<task-id>`, `.loop-complete-<task-id>`, `.loop-halt-<task-id>` — see Phase 9.5 for definitions. When goal-supervised mode is opted into at Phase 9.6, this directory additionally holds per-task goal-supervised state: `.goal-active-<task-id>` (active marker), `.iteration-summary-<task-id>-<iter-n>.md` (the structured per-iteration summary the judge reads), `.evaluator-feedback-<task-id>.md` (accumulated judge `{verdict, reason}` responses). The `decisions-log-<task-id>.md` and `loop-final-<task-id>.md` files are shared with loop mode — same filename convention, same operator-facing audit purpose. See Phase 9.6 for definitions. When queue mode is opted into at Phase 9.7, queue-mode artifacts live in a sibling directory (`.claude/queue/`) rather than in `.claude/sessions/`, because they are queue-scoped (one set per queue run) rather than session-scoped (one set per session) — see Phase 9.7 for the directory layout. The queue runner does not write to `.claude/sessions/` directly; it dispatches the per-task wrappers, which continue to write their per-task artifacts into `.claude/sessions/` as before.
...
1. Write `CLAUDE.md` at repo root with:
   - References to all created steering docs (product, tech, deps, secrets, structure, principles, ci-cd, tools, spec-versioning, and — when `design_steering_enabled` — design — count varies based on which phases ran).
   ...
   - **Explicit escalation criteria** — when the agent must stop and ask the human. Items marked **🔔** trigger the decision-required audio alarm (urgent — agent is blocked); others get in-chat output only. **Inside an autonomous mode** (when `.claude/sessions/.loop-active-<task-id>` *or* `.claude/sessions/.goal-active-<task-id>` exists), the urgent items still hard-block but additionally cause the agent to write `.loop-halt-<task-id>` and end the turn so the wrapper can halt the loop; the non-urgent items are routed to `decisions-log-<task-id>.md` instead of producing chat output. Behavior is identical between loop mode and goal-supervised mode for these criteria — both modes share the same `.loop-halt-<task-id>` and `decisions-log-<task-id>.md` filename conventions, and the same urgent vs non-urgent routing:
     ...
     - The drift detector has fired Alert 3 (firm). Tier 3 is enforced: the agent writes `.claude/sessions/<timestamp>-checkpoint.md` (standard synopsis schema; **[X-33]** `<timestamp>` from the clock — `date -u +%Y-%m-%dT%H%MZ` — never from model memory, per Phase 7 step 6), and is then hard-blocked from further tool calls until the operator runs `/clear`. The agent's responsibility is to produce a complete, useful checkpoint; the operator's responsibility is to run `/clear` and `/resume`. *(Inside an autonomous mode: the agent writes the checkpoint and ends the turn; the wrapper starts a fresh session primed with the new checkpoint, no operator action needed — see Phase 9.5 for loop mode and Phase 9.6 for goal-supervised mode.)*
   - When triggering the decision-required alarm, the agent writes to `.claude/sessions/.decision-pending-<session-id>` with: timestamp, escalation reason, what the agent was about to do, what input it needs. **[X-30]** This write is the operator's only record of *why* an unattended run stopped, so the alarm hook is required never to truncate the file (§6.E) — verify the two agree before shipping either, because through 2.6.1 they did not.
...
9. **Commit the golden set [LIT-06] (only if any autonomous mode was opted into).** Promote 2–3 of the known-good, loop-eligible, trivially revertible smoke tasks above (stages 3–5; or generate equivalents if the smokes were declined) into a committed **golden set** — task definitions the operator re-runs on demand as a regression harness. On any Assumption-Ledger trigger (pinned-model or runtime-floor change), the operator re-runs the set **through each *enabled* autonomous mode** and compares the [LIT-08] metrics (outcome, tokens, iterations-vs-cap, format-validity) against a stored baseline — converting the ledger's fail-loud *notice* into a *measurement*. Golden runs carry their own iteration cap of 2–3: a golden task that needs more iterations *is* the regression signal, and the cap bounds the cost. **Stated cost:** worst case ≈ 3 tasks × enabled modes × 2–3 iterations of full priming per trigger — real but bounded tokens; know this before adopting the practice. For telemetry-on operators, TEL-01 supplies *cost-shape* evidence for two ledger rows; golden runs are the behavioral complement (does the loop still converge?), not a duplicate channel. The same runs serve as the [LIT-02] consolidation procedure's pre/post check (Phase 10). Cap the set at 3; treat golden-set staleness as an ordinary spec-versioning event. (No collision with GR2-06 — different mechanism; note only that the runtime-floor change the owed seam re-cut would produce is itself a golden-run trigger.)

**Exit criteria:** Either smoke-test artifacts exist (stage 1 minimum, stage 2 ideally, stage 3 when loop mode is opted in, stage 4 when goal-supervised mode is opted in, stage 5 when queue mode is opted in) and look right, or operator opted to skip. When any autonomous mode is enabled: golden set committed, or explicitly declined [LIT-06].
...
3. **Loads the priming context.** Concatenates, in order: the task's `progress.md` (the status + failed-approaches ledger, read before the task brief so prior dead ends are visible), the task's `requirements.md` slice, the task definition itself **with `loop_max_iterations` filtered from the primed copy [LIT-07]** (the field stays in the committed task file and the task-definition schema; the wrapper's cap enforcement is unchanged), the loop-completion criteria, the most recent checkpoint (if resuming), and the project's standard `CLAUDE.md`. The primed context includes the sentence: *"Don't ration or count iterations; the harness manages all budgets and will end the loop when appropriate."* [LIT-07]
...
When any of these fire inside loop mode, the agent writes the existing `.decision-pending-<session-id>` file (v1.5.0 mechanism), writes an additional `.loop-halt-<task-id>` marker, and exits. The wrapper sees the halt marker and exits the loop entirely — no restart. The operator returns to a halted loop with the decision-pending file ready for action. **[X-30]** "Ready for action" means *carrying the four fields the agent wrote*, which is why §6.E forbids the alarm hook from truncating it: an overnight halt whose record is a zero-byte file delivers a filename and nothing else — precisely the information the halt exists to convey.
...
**The cap is deliberately absent from the primed slice [LIT-07].** Enforcement is the harness's job; a prompt-visible budget invites rationing against it (the budget-anxiety mechanism — the "context anxiety" lineage the v2.3.0 note cites, Cognition's term). Redaction reduces prompt *salience* only — the cap stays truthful and workspace-readable in the committed task file, in committed `loop-config.md`/`goal-config.md`, and in `learnings/mode-selection.md`'s iterations-vs-cap entries [LIT-08] — which is defensible non-disclosure consistent with the safety-net/success-signal split above, not concealment. The redaction reaches every primed artifact: `progress.md` is primed *first* each iteration, so its `Status` line must not restate the cap — the emitted progress-template's exemplar is cap-free by design ("iteration 3", never "iteration 3 of 10") [LIT-07]. The why-redacted rationale is recorded operator-visibly in the config comment blocks so a future maintainer can tell this deliberate absence from an oversight and does not "fix" the unfiltered task definition back into priming. Recorded in the [Assumption Ledger](#assumption-ledger): calibrated against the current tier; a model generation that plans *better* with a known budget flips this default.
...
The five design principles from Phase 9.5 carry forward unchanged. Four new principles apply to goal-supervised mode:
...
- **Haiku-tier components get single-step, schema-constrained jobs only — never multi-step scaffolds. [LIT-10]** The protocol already behaves this way (structured `{verdict, reason}` judge output, the iteration-summary enforcement hook, retry-once-then-halt); this elevates the behavior from accident of design to stated invariant so future edits cannot regress it silently. (The ≥7B-or-reasoning-model floor behind it is the LIT proposal's synthesis from P3's sub-7B data — labeled as inference, not P3's statement.)
...
**Field provenance [LIT-08] (transcription, not new instrumentation).** The four metric fields are wrapper-promised or trajectory-derivable — with one honest caveat. Goal mode's `loop-final-<task-id>.md` promises elapsed/turns/tokens/evaluator-calls and `halt_reason`; loop mode's loop-final promises the iteration history and the required `Trajectory` line but names no tokens field — loop-mode tokens come from the retained trajectory JSONL or are recorded as unavailable, and format-validity is structurally N/A for loop-mode rows, and the template says so rather than implying uniform recordability. The caveat: the `summary_failure_count` counter behind format-validity is a §6.C obligation of the *operator-completed* wrapper, not yet implemented by any emitted artifact (backlog O-2, open) — until the operator's loop counts it, record format-validity as unavailable, the same fallback tokens has. **[PAR-05] This transcription is the telemetry-off path:** when the operator has opted into TEL-01, `.claude/steering/telemetry.md` already names `claude_code.token.usage` sliced by `agent.name` as queryable evidence, and the review may query instead of transcribe — one evidence channel, stated, not two unreconciled ones. With ~20 entries the calibration review can run a per-project score-vs-cost comparison across mode×model pairs — the right-sized private version of a public leaderboard.

After ~20 entries, the operator may invoke a calibration-review skill that surfaces the accumulated answers, highlights any classifier-recommendation/operator-choice divergences and any "felt wrong" patterns, and proposes adjustments to the recommendation rule's flip-properties for this specific project. The operator approves, vetoes, or refines. This is the same posture as the existing `learnings/` mechanism: the protocol provides starting structure, the operator refines over time. (No calibration-review skill is installer-emitted; the review is the operator's pass over this ledger, with or without a skill the operator added. [LIT-09])

**Complexity-downgrade rule [LIT-09].** Each scaffold layer must pay for itself in the ledger. If the ~20-entry review shows no case where the goal-supervised judge's verdict diverged *usefully* from the deterministic gates, the review proposes downgrading that task class to loop mode; the same test applies one level down (loop mode vs operator-only-with-gates). "Usefully diverged" is judged with the entry template's *Felt right? = "yes but the judge was noisy"* answer, not raw divergence counts — divergence alone cannot separate judge-caught-a-real-issue from judge-was-wrong, which is precisely the distinction the `goal-condition-suspect` halt exists to surface. Recommendations, not decisions: the operator approves, mirroring the trust ramp's posture in the opposite direction. Premature downgrade off a small sample is bounded by the ~20-entry threshold and the operator veto.
...
The principles from Phase 9.5 (per-task scoping, opt-in per task, enforcement guarantees survive, decisions logged not skipped, the loop is bounded) carry forward unchanged. The principles from Phase 9.6 (judge advisory not authoritative, judge sees artifacts not transcripts, Haiku-tier components single-step and schema-constrained only [LIT-10], third tier not replacement) also carry forward unchanged. Four new principles apply at the coordination layer:
...
5. Note explicitly:
   ...
   - `learnings/` is for cross-feature memory. Add to it after each feature. When `learnings/` or `sessions/` growth starts crowding `/resume` context, run the consolidation procedure below [LIT-02].
   - **Memory consolidation (operator-run, reversible, eval-gated) [LIT-02].** Scope: `.claude/learnings/` (and its README convention) only — `.claude/specs/INDEX.md` is the living task board, not a memory index, and is never reorganized by this procedure. Vendor-neutral: it constrains *any* consolidation mechanism, operator-run or auto-memory. Steps: (1) **snapshot = a git commit (or tag) of the pre-consolidation state** — `learnings/` is a committed directory, so git is the snapshot and `git revert` is the rollback; no filesystem copy is parked anywhere the 7-day state purge or the GR2-02 log-pruning obligation could reach, because no filesystem copy exists; (2) reorganize into a parallel copy — never in place; (3) diff for dropped facts against the [LIT-01] preservation invariant; (4) run the [LIT-06] golden set against both stores; (5) promote only if the new store scores no worse — **ties promote**, acceptable solely because the git snapshot makes every promotion reversible; retain the pre-consolidation commit reference in the promotion's decision-log entry until operator approval. The eval gate is only as good as the golden set (small-N; noise can pass a tie): its job is catching gross degradation, and the git snapshot keeps every promotion reversible regardless.
   ...
   - **If goal-supervised mode was opted into:** start operator-in-the-loop for the first 10–20 tasks anyway. Use the time to develop a feel for how `spec-decompose` classifies your tasks (does the sixth criterion correctly identify goal-judge-able tasks?), how the recommendation rule behaves on eligible-for-both tasks (does it pick the mode you would have picked?), and how Haiku judges your codebase (are its "no" reasons useful, or noise?). Record per-task assessments in `learnings/mode-selection.md`. After 20 tasks, review the accumulated evidence and adjust the recommendation rule if needed — including the complexity-downgrade question [LIT-09]: a scaffold layer that never usefully diverged from the deterministic gates should be proposed for downgrade, one level at a time. Once you trust the classifier and the judge, run goal-supervised mode on tasks where it's recommended. Each run produces `loop-final-<task-id>.md`, `decisions-log-<task-id>.md`, `.iteration-summary-<task-id>-*.md` files, and `.evaluator-feedback-<task-id>.md` — review all four artifact types before merging, especially the disagreement entries. If a task halted with `goal-condition-suspect`, treat that as the protocol working as designed: the goal condition or the judge needs human attention, not a bigger model or more iterations.
...
**Problem.** `.bootstrap-state.json` is load-bearing across resume, the in-flight lists, `queue_runs_history`, the crash-recovery close-out, and the "enable a mode later" flows. The authoritative document specifies the *current* shape but defers all migration/compat behavior entirely to `Bootstrap-Protocol-Companion-v2-8-0.md`'s "Migration notes." There is no statement in Bootstrap-Protocol-v2-8-0.md of how a runner or wrapper must behave when it encounters a state file written by a different `bootstrap_protocol_version` (forward-compat: newer file, older tool; backward-compat: older file, newer tool) — e.g., unknown `exit_reason` values, missing `queue_runs_history`, absent mode flags.
...
### Autonomous-loop spec-review proposals (2026-09-06, NOT yet adopted)
...
Two entries deliberately sit next to already-open items — ALS-3 beside B-2, and
ALS-7 beside B-7 — and each says why it is not the same question.

### ALS-1 — `--worktree` re-roots the project directory, so the wrapper and the agent do not share a sentinel namespace `[ALS-1 — owner decision pending]`

**Problem.** Phase 9.5's `[IC-6]` routing note states that the native worktree mechanism "isolates the working directory only" — and the emitted wrapper it governs restates that as "a worktree is a drift-prevention boundary, NOT a security boundary" (`lib/templates.py:7540`; the phrasing is the template's, not this document's). MEASURED on 2.1.263 — a `PreToolUse` hook dumping its payload and environment under `claude --print --worktree wt-T-001` reports `CLAUDE_PROJECT_DIR`, the hook process cwd, and the payload's `cwd` all equal to `<repo>/.claude/worktrees/wt-T-001`. The flag re-roots the project directory; it does not merely move the working directory. The exit protocol in the same phase is a cross-process filesystem handshake — the wrapper decides what happened by testing for `.loop-complete-<task-id>`, `.loop-halt-<task-id>` and `.drift-tier3-<session-id>` — and every path the document hands the *agent* for those writes is relative and unrooted, while the wrapper self-locates to the main checkout. The two resolve the same relative path in different trees.

**Why it matters.** The failure is silent and it is in the halt direction. An agent that hits an urgent-escalation criterion writes `.loop-halt-<task-id>` into the worktree and ends its turn; the wrapper polls the main checkout, finds nothing, matches the "none of the above" row of the iteration-end table, and treats a halt as an ordinary continuable iteration — it re-primes and keeps running unattended. The `[W-1]` note is the one place the document confronts the worktree cwd boundary and it is scoped explicitly to *gate commands* ("a plain `pytest -q` inherits the hook process's cwd"); it never reaches sentinel writes. Nothing else in Phases 9.5–9.7 names the tree in which a sentinel lands.

**Specific change I would make (if adopted).** Two edits. (1) Correct the `[IC-6]` claim: state that `--worktree` re-roots `CLAUDE_PROJECT_DIR` as well as the working directory, and that anything the wrapper and the dispatched agent must both see is therefore *not* shared by default. (2) Give the exit protocol an explicit root: state that all wrapper-agent handshake artifacts resolve against the **launching** wrapper's project directory, and that a dispatch which re-roots the agent must either pass that root to the agent (an absolute path in the primed context) or read the sentinels from the worktree it created. Name which of the two the protocol chooses.

**Cost / what it could break.** Fixing this by passing an absolute root into the primed context puts a machine-specific path into a context slice the document otherwise keeps portable, and it is a new normative field on the priming contract. Fixing it by having the wrapper read the worktree instead makes the wrapper depend on the worktree's location, which the native mechanism owns and may change. There is also a scope limit worth stating plainly: this was measured on the **CLI-flag** route only. Whether an `isolation: worktree` subagent behaves the same way is a separate question and is not settled by this measurement.

**OPEN QUESTION for the human owner.** Should the handshake artifacts be defined as resolving against the wrapper's project directory (agent receives an absolute root), or should the wrapper follow the agent into the worktree? And is the `[IC-6]` "isolates the working directory only" sentence a claim the protocol wants to keep for the subagent route even though it is false for the CLI-flag route?

### ALS-2 — Two of the three per-task sentinels have no lifecycle, so a re-looped task can terminate before it starts `[ALS-2 — owner decision pending]`

**Problem.** The wrapper's terminal-exit step removes exactly one sentinel, `.loop-active-<task-id>`. No clause in this document, the companion, or the seam contract removes, expires, or iteration-scopes `.loop-complete-<task-id>` or `.loop-halt-<task-id>`, and no clause has the wrapper clear or ignore a pre-existing one at startup — the claim sequence refuses only when `.loop-active-<task-id>` already exists, and the deliverable contract's startup obligation is scoped to a different sentinel set (the root and queue-scoped halt files). Both unremoved sentinels are task-scoped and agent-written, so once written they satisfy their own tests until the 7-day state purge.

**Why it matters.** The document prescribes re-running the same task: "For any logged decision the operator disagrees with: they edit the relevant code/spec, set a flag, and re-loop the task." That second run begins with the first run's completion sentinel on disk, and the first row of the iteration-end table fires on it — terminal success, write the final summary, exit — **before iteration 1 has done any work.** The operator's correction is silently discarded and the run reports success. The halt sentinel has the mirror-image problem: a task halted once is unstartable until the purge.

**Specific change I would make (if adopted).** Add the two missing lifecycle statements. (1) Terminal exit removes `.loop-complete-<task-id>` and `.loop-halt-<task-id>` alongside `.loop-active-<task-id>`. (2) The claim sequence treats a pre-existing completion or halt sentinel for the task it is claiming as **stale by default** and clears it, or refuses with a named reason — the document should pick one and say which. If "refuse" is chosen, the re-loop flow needs an operator-facing way to clear the sentinel, and that command belongs in the same paragraph.

**Cost / what it could break.** Clearing on claim removes the only durable record that a prior run completed or halted, which the queue runner's own classification step reads. If the queue path depends on that file persisting past the wrapper's exit, then "remove at terminal exit" is wrong for queue-dispatched tasks and right for manually-dispatched ones — a mode-dependent lifecycle, which is more surface than it first appears. The interaction with `loop-final-<task-id>.md`, which *is* a committed audit record, needs to be stated in the same edit so the two do not disagree.

**OPEN QUESTION for the human owner.** Should a pre-existing completion/halt sentinel be cleared automatically at claim time, or should the wrapper refuse and require an explicit operator clear? And does the queue runner require either sentinel to survive the wrapper's exit — if so, which, and for how long?

### ALS-3 — The two mandated state-file idioms do not compose, so the no-lost-entries guarantee fails between cooperating writers `[ALS-3 — owner decision pending]`

**Problem.** Recovery & State mandates both an advisory lock on `.claude/.bootstrap-state.json` and the tmpfile-then-rename write idiom for that same file, and builds guarantee (b) — that the in-flight lists never lose entries under concurrent updates — on the pair. The two do not compose. An advisory lock binds to the open file description, and therefore to the inode; `rename` replaces the path's inode. Once any writer commits, every later writer opens a *different* inode and its lock excludes nobody. MEASURED: three concurrent writers staggered 0.15 s apart, at the operating point this document itself documents (two per-task wrappers plus the queue runner, all read-modify-writing the state file), retained **2 of 3** entries in the specified form; the identical lock with an in-place write retained **3 of 3**. One variable moved, with the reference arm present.

**Why it matters.** This is not the advisory-lock concern already recorded as B-2. **B-2's case requires a writer that never takes the lock** — the sanctioned manual dispatch that bypasses the convention. This one breaks with **every writer cooperating and taking the lock exactly as specified**: the two idioms the document mandates in adjacent sentences cancel each other. B-2 and this entry can both be true, and adopting B-2's answer would not repair this. A lost in-flight entry is not a cosmetic bookkeeping error — it is how the combined-concurrency cap is enforced and how the crash-recovery close-out finds dangling runs.

**Specific change I would make (if adopted).** Pick a pair that composes and mandate it. Either (a) lock a dedicated sidecar (e.g. `.claude/.bootstrap-state.json.lock`) that is never renamed, keeping tmpfile-then-rename for the data file — the lock's inode is then stable across writes; or (b) keep locking the data file and drop `rename` in favour of an in-place rewrite under the lock, accepting the loss of write-atomicity against a crash mid-write. State the tradeoff either way: (a) keeps crash-atomicity and needs one more file; (b) keeps one file and reintroduces a torn-write window.

**Cost / what it could break.** Option (a) adds a file to the emitted tree and to the state-file contract, and every consumer that locks the data file directly becomes non-conformant. Option (b) trades a real property away — a crash between truncate and write leaves the state file corrupt, which is exactly what tmpfile-then-rename was chosen to prevent — so it should not be adopted without saying what recovers from a torn state file. The measurement is an inode-semantics argument and holds wherever the advisory lock is the POSIX-style one; the document's named Windows equivalent is **untested, not refuted**, and adopting either option should say whether the Windows branch inherits the same rule.

**OPEN QUESTION for the human owner.** Sidecar lock (a) or in-place write (b)? And if (a), is the lock file part of the emitted tree's contract — i.e. must a conformant installer create it — or is it an implementation detail the wrapper may choose?

### ALS-4 — The project-root halt sentinels never reached the termination vocabulary they are supposed to be reported through `[ALS-4 — owner decision pending]`

**Problem.** `IC-2` introduced project-root `.halt` and `.halt-hard` sentinels, and the wrappers' deliverable contract requires honouring them at startup **and** requires the emitted comment contract to enumerate the full `exit_reason` value set from Recovery & State, "so an operator-completed dispatch loop implements the whole enum rather than the subset the skeleton exercises." Recovery & State was never updated for those sentinels. Searching this document for `halt-hard` returns exactly two sites — the `IC-2` introduction and the deliverable contract — and the enum's `manual-halt-sentinel` value is defined against the *queue-scoped* sentinel observed at a dispatch decision point. Repo-wide the string appears in fourteen files, and the two that could carry the binding do not: the seam contract gives the sentinel a shared-surface row with no `exit_reason`, and the emitted wrapper template implements the behaviour with no enum value.

**Why it matters.** The two requirements conflict, and an implementer cannot satisfy both. Handed Phases 9.5–9.7 plus Recovery & State, there is no answer to "what `exit_reason` does a runner record when `<project-dir>/.halt-hard` appears mid-run" — the vocabulary the contract points at does not contain the case the contract mandates handling. Every unattended run stopped by the root sentinel is therefore recorded under a value that names something that did not happen, or under none at all.

**Specific change I would make (if adopted).** The cheaper of two options: add one sentence to the `manual-halt-sentinel` definition binding it to the root sentinels as well as the queue-scoped ones, distinguishing graceful (`.halt`, honoured at the next boundary) from immediate (`.halt-hard`) in the same sentence. The alternative — a fourteenth enum value for the hard-halt case — is more faithful but moves the "exactly 13" pin the test suite carries, and the prior decision recorded in the backlog declined a fourteenth value on the grounds that the proposed one named an authoring state. That reasoning does not dispose of this case, because a root hard-halt observed mid-run *is* a run outcome — so the owner should decide rather than inherit it.

**Cost / what it could break.** Binding both sentinel sets to one value makes the morning-after summary's "Ended because" line less precise: an operator reading `manual-halt-sentinel` cannot tell whether the run stopped gracefully at a boundary or was cut immediately, which is exactly the distinction they need when deciding what is safe to keep. Adding the fourteenth value avoids that but breaks a pinned count and every consumer that enumerates the enum.

**OPEN QUESTION for the human owner.** Extend `manual-halt-sentinel` to cover the root sentinels, or add a distinct value for the immediate hard-halt? If the former, is the graceful/immediate distinction recoverable anywhere the operator will actually look?

### ALS-5 — The judge's primary input names a baseline no artifact records `[ALS-5 — owner decision pending]`

**Problem.** Phase 9.6 specifies that the judge "reads the persistent state — the git diff since loop start, the agent's structured iteration summary, the current test output — and renders a verdict on the goal condition," and argues this is more reliable than evaluating in-session conversational text. Nothing records the revision that "since loop start" is measured from. The in-flight entry schema carries `task_id`, `iteration`, `started_at` and an optional worktree path — a wall-clock timestamp, never a commit. Searching this document, the companion and the seam contract for a loop-start revision under any of the obvious spellings returns only the sentence that needs it. Phase 9.5 explicitly permits the agent to commit mid-iteration, so the naive reading — a plain working-tree diff — prints nothing at all once the agent has committed. Loop mode carries the same hole where it primes the next iteration "plus the current diff."

**Why it matters.** The judge is the mechanism the goal-supervised mode exists for, and this is its primary input. An unrecorded baseline does not fail loudly: it yields an empty or wrong-sized diff, and the judge renders a verdict on it anyway. The document's own argument for the judge's reliability — that it reads artifacts rather than conversation — depends on the artifact being computable.

**Specific change I would make (if adopted).** Add a base revision to the in-flight entry schema (e.g. `base_rev`, captured at claim time before the first dispatch) and define the judge's input as the diff from that revision to the current working tree, committed and uncommitted. Mirror the same field into the loop-mode priming sentence so both modes name one baseline. Say explicitly whether the baseline is re-captured at each iteration or fixed for the task.

**Cost / what it could break.** This is a state-schema addition, so it touches the file whose compat rules B-4 already observes are undefined — a state file written by a version that lacks the field must have a stated fallback, and "recompute from `started_at`" is not one. Fixing the baseline for the whole task means the judge sees cumulative work and its input grows with iteration count, which is a cost the token-budget question below has to absorb; re-capturing per iteration keeps it small but makes the goal-condition verdict blind to work done in earlier iterations.

**OPEN QUESTION for the human owner.** Fixed-at-claim or re-captured-per-iteration? And what must a wrapper do when it reads an in-flight entry that predates the field — refuse, or fall back to a stated approximation?

### ALS-6 — The queue file's run-start reset erases the cross-run fact that readiness depends on `[ALS-6 — owner decision pending]`

**Problem.** The queue file's "Completed this run" and "Halted this run" sections are both specified as populated by the runner at terminal exit and **cleared at run start**. Readiness, however, is defined across runs: a task is dispatchable only when every `blocked_by` predecessor has terminated successfully "in the current or a prior queue run." That is a cross-run fact with no durable home. After the reset, a predecessor that halted in run N is, in run N+1, neither in the completed section nor the halted one.

**Why it matters.** The successor reads as ready and the runner dispatches it — unattended, on top of a dependency that failed. This is the ordinary way an overnight run ends (a task halts, the operator fixes it in the morning, the queue runs again), so the failure is not an edge case; it is the second run of any queue that ever halted. It also defeats the purpose of `blocked_by`, which exists to stop exactly this.

**Specific change I would make (if adopted).** Either (a) do not clear the halted section at run start — clear only the per-run *narrative* sections and keep terminal outcomes as an accumulating record keyed by task; or (b) define readiness against a durable source instead of the queue file's per-run sections, naming that source (the state file's history, or the per-task final summary). (a) is the smaller edit; (b) is the one that survives an operator hand-editing the queue file, which the document elsewhere sanctions.

**Cost / what it could break.** Under (a) the queue file grows without bound across runs and the operator's own editing surface becomes a record that must not be casually pruned — the document currently treats the queue file as operator-editable working state, and this makes part of it load-bearing history. Under (b) readiness stops being answerable by reading the queue file alone, which is the property that makes the file reviewable by a human in the morning.

**OPEN QUESTION for the human owner.** Keep the cross-run record in the queue file (and accept unbounded growth plus an editing hazard), or move readiness onto a durable source and accept that the queue file no longer answers "what is ready" on its own?

### ALS-7 — Queue mode's token budget has no input on the loop-mode path `[ALS-7 — owner decision pending]`

**Problem.** The queue runner's token budget is specified as tracked "via per-task summaries," with the runner relying on "the per-task wrappers reporting consumption" — the runner deliberately invokes no model with token counting. The calibration-ledger provenance note states of the same artifact that **loop mode's final summary names no tokens field**, and that loop-mode tokens "come from the retained trajectory JSONL or are recorded as unavailable." Nothing assigns the runner the job of parsing trajectory JSONL, and that note assigns the route to the calibration review rather than to the runner.

**Why it matters.** On the loop-mode path the runner's only specified source carries no token count, so the budget cannot be enforced — and a budget that silently does not apply is worse than an absent one, because the operator set a number and believes it is holding. The two statements are three sections apart and neither cites the other, so an implementer reading either one alone will not notice. This is adjacent to B-7, which asks which `exit_reason` a *time* budget writes on the operator-only pause; that question presumes the budget mechanism works, where this one asks whether the token budget has an input at all on one of its two paths.

**Specific change I would make (if adopted).** Pick one and state it where both readers will meet it. Either (a) add a tokens field to loop mode's final summary, making the wrapper report consumption exactly as the goal-mode summary already does — the runner's stated source then works on both paths; or (b) declare the token budget goal-mode-only, and have the runner refuse or warn at startup when a token budget is set and any queued task is loop-mode. (b) is honest and cheap; (a) is what the budget's own description already assumes.

**Cost / what it could break.** Under (a) the wrapper must obtain a token count, and the only stated source is the retained trajectory stream — so a fail-loud path is needed for the case where retention is off or the stream lacks the field, and "recorded as unavailable" then has to mean something specific to the budget check rather than being a free-text note. Under (b) an operator who runs a mixed queue loses budget enforcement on part of it, and the refuse-or-warn choice determines whether that is a startup failure or a silent partial guarantee.

**OPEN QUESTION for the human owner.** Add the tokens field to loop mode's summary, or scope the token budget to goal mode? If the latter, should a token budget set against a loop-mode queue refuse at startup, or warn and continue with the budget unenforced on those tasks?
